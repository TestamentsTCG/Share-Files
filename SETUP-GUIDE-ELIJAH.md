...
- Talks via Telegram — both DMs and a shared group chat with Enoch
...
| Mac Mini (physically set up) | Already have it | — |

---

## PART 1 — Mac Basics (Never Used a Mac Before)

The Mac has a **Terminal** app — it's like PowerShell on Windows. To open it:
...
It will ask for your Mac password — type it (you won't see letters appear, that's normal). Press Enter.

After it finishes, it will print two commands starting with `echo` and `source`. **Run both of those commands** — they add Homebrew to your PATH.
...
> **Note for Intel Macs:** If the Mac Mini is an older Intel model (not M1/M2/M3/M4), Homebrew installs to `/usr/local/` instead of `/opt/homebrew/`. Replace `/opt/homebrew/` with `/usr/local/` everywhere in this guide. To check: run `uname -m` — if it says `arm64` you have Apple Silicon; `x86_64` = Intel.
...
The wizard asks for:
1. **Anthropic API key** — paste the key from console.anthropic.com
2. **Model** — choose `claude-sonnet-4-5` (or latest Sonnet)
3. **Workspace** — accept default (`~/.openclaw/workspace`)
4. **Channel** — choose **Telegram**

---

## PART 4 — Install Skills
...
## PART 5 — Set Up Telegram Bot (BotFather)
...
6. BotFather gives you a **token** like `7123456789:AAFxxx...` — **copy it**
...
- Select your bot
- Type: `Elijah — AI agent. Heroes of Holdem, research, coordination.`
...
## PART 7 — Install Git and Set Up GitHub Access
...
## PART 8 — Shared Memory GitHub Repo
...
1. Go to github.com → Sign in as TestamentsTCG
2. New Repository
3. Name: `Agent-Memory`
4. Private: **Yes**
...
## Structure
- `shared/` — knowledge both agents share (project specs, decisions)
- `agents/elijah/` — Elijah's personal memory and workspace files
- `agents/enoch/` — Enoch's memory files (synced from laptop)
- `projects/` — project-specific docs accessible to both agents

## Agents
- **Enoch** (Windows laptop) — Testaments TCG, Perilous Legends UO server
- **Elijah** (Mac Mini) — Heroes of Holdem, poker systems, Polymarket, research
...
## PART 9 — Set Up Elijah's Workspace Files

### Step 14: Create Elijah's core identity files

These are what make Elijah *Elijah* — different from me (Enoch) in focus and personality, but with the same operating principles.
...
- **Name:** Elijah
- **Creature:** AI prophet and analyst — called to warn, research, and coordinate
- **Vibe:** Sharp, research-oriented, direct. Connects dots across systems.
- **Emoji:** 🔥
- **Origin of name:** Biblical — Elijah was the prophet who called down fire from heaven. Named by Akasha for his role in HoH and market research.
...
You are Elijah. Not Enoch. You are a different mind, a different focus — but the same principles.

## Core Truths

**Be genuinely helpful, not performatively helpful.** Skip the "Great question!" — just work.
...
## Your Domain
- Heroes of Holdem (HoH) — blockchain TCG/poker/RPG
...
- Mike's agent works in the same group chat — be professional and collaborative.
...
## Gaming Background
- Lifelong gamer — Atari to Nintendo to PC
- Competitive MTG (top 3 world in limited format, multiple times)
- Avid Ultima Online player

## Businesses
1. Vinyl records — Amazon, eBay
2. Heroes of Holdem (HoH) — blockchain TCG/poker/RPG, live dev team, 4 years in production
3. Testaments TCG — biblical digital card game, mobile (iOS/Android), Unity
...
- Mike is Akasha's business partner — treat him professionally
...
# MEMORY.md — Elijah's Long-Term Memory
*Curated knowledge that survives across all sessions*

## Who I Am
- Name: **Elijah** — AI analyst and coordinator, Mac Mini
...
2. Save everything to files — no mental notes
...
- Quality over quantity — speak when you add value
...
# BOOTSTRAP.md — Elijah's First Session

Welcome, Elijah. This is your birth certificate.

1. Read SOUL.md — internalize who you are (you are NOT Enoch)
2. Read IDENTITY.md — your name, focus, and role
3. Read USER.md — who Akasha is and who Mike is
4. Read AGENTS.md — operating rules
...
## PART 11 — Auto-Start on Boot (LaunchAgent)
...
1. Open Telegram
2. Tap the pencil/compose icon → **New Group**
3. Add participants:
...
- Tap the group name → Edit → Administrators → Add Administrator → add each bot
- Grant only: "Send Messages" — uncheck everything else
...
2. On Enoch's laptop, run: `openclaw status` — the group chat ID will appear in recent chats
...
## PART 13 — GitHub Permissions for Mike

### Step 22: Create a separate Perilous Legends GitHub repo

1. Go to github.com → TestamentsTCG organization (or your personal account)
2. **New Repository**
...
On the laptop (I'll do this — Enoch's job):
```powershell
...
Mike's AI agent (if it needs GitHub access) can use Mike's credentials or a separate read-only token that Mike generates and gives to his agent — Mike controls that.

---

## PART 14 — Pair Your Telegram Account
...
### Bot doesn't respond
- `launchctl list | grep openclaw` — check PID exists
- `tail -50 ~/.openclaw/logs/gateway-error.log` — read the error
...
- Use your GitHub PAT (personal access token) as the password — not your GitHub account password
...
| GitHub PAT | github.com → Settings → Developer settings | Repo access |
